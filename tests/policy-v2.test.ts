...
test("untrusted provenance cannot become a CI pass", () => {
  const parsed = validateContextV2({
    ...
    provenance: { adapter_id: "unknown", mode: "host_asserted" },
  });
  const result = evaluateGateV2({
    tool: "jev_review",
    modelAction: "auto",
...
test("model review/escalate cannot be promoted by deterministic policy", () => {
  ...
  const review = evaluateGateV2({
    tool: "jev_review",
    modelAction: "review",
    context: parsed,
    trustedAdapterIds: ["jev-mcp-ci"],
  });
  const escalate = evaluateGateV2({
    tool: "jev_review",
