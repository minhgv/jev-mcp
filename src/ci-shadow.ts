...
export async function runCiShadow(argv: string[]): Promise<void> {
  ...
  try {
    ...
    process.stdout.write(`${JSON.stringify({ shadow: true, would_block: evaluation.action !== "auto" || !(evaluation.gate_eligibility as { eligible: boolean }).eligible, evaluation }, null, 2)}\n`);
  } catch (error) {
