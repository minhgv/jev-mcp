...
import { protectedClassesForPath, type ProtectedClass } from "./path-policy.js";
import { CONTEXT_SCHEMA_VERSION_V2, validateContextV2, type ChangeContextV2 } from "./contracts.js";
...
export function redactText(text: string): string {
  return text
    .replace(/(\b(?:token|api[_-]?key|password|secret|private[_-]?key)\b\s*[:=]\s*)([^\s"'`]+)/gi, "$1[REDACTED]")
...
export function buildChangeContext(snapshot: CollectorSnapshot, options: CollectorOptions): ChangeContextV2 {
  ...
  const contextWithoutDigest = {
    ...
    sections: [
      { id: "request", kind: "request" as const, content: snapshot.request ?? "Unspecified change request", available: true, truncated: false, redacted: false },
      { id: "diff", kind: "diff" as const, content: redactedDiff, available: true, truncated: false, redacted: redactedDiff !== snapshot.diff },
      ...(redactedTests === undefined ? [] : [{ id: "tests", kind: "tests" as const, content: redactedTests, available: true, truncated: false, redacted: redactedTests !== snapshot.tests }]),
    ],
...
function runGit(cwd: string, args: string[]): string {
  try {
    return execFileSync("git", args, { cwd, encoding: "utf8", maxBuffer: 2_000_000, stdio: ["ignore", "pipe", "pipe"] }).trim();
  } catch (error) {
...
function parseStatus(nameStatus: string, porcelain: string): CollectorFile[] {
  ...
  for (const line of porcelain.split("\n").filter((item) => item.startsWith("?? "))) {
    ...
    if (!files.some((file) => file.path === path)) files.push({ path, change: "added", binary: false, generated: false });
  }
  return files;
}
